...
function Table({ className, ...props }: React.ComponentProps<'table'>) {
  return (
    <div
      ...
      className="relative w-full overflow-x-auto rounded-2xl border border-border/60 bg-card shadow-soft-xs"
    >
      <table
        data-slot="table"
...
function TableHeader({ className, ...props }: React.ComponentProps<'thead'>) {
  return (
    <thead
      ...
      className={cn('bg-muted/60 [&_tr]:border-b border-border/60', className)}
      {...props}
    />
  )
}
...
function TableRow({ className, ...props }: React.ComponentProps<'tr'>) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        'hover:bg-muted/40 data-[state=selected]:bg-muted/60 border-b border-border/40 transition-colors',
        className,
      )}
      {...props}
    />
